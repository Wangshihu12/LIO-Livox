                                               ...
                                               std::vector<int> &pointsLessFlat)
{
  ...
  for (int j = 0; j < thPartNum; j++)
  {
    ...
    for (int k = sp; k <= ep; k++)
    {
      ...
      if (cloudCurvature[idx] < 0.7 * thFlatThreshold * cloudDepth[idx] * thFlatThreshold * cloudDepth[idx] && sharpestPickedNum <= 3 && cloudReflect[idx] > 20.0)
      {
        sharpestPickedNum++;
  ...
  for (int i = 5; i < cloudSize - 5; i += count_num)
  {
    ...
    // left curvature
    float ldiffX =
        _laserCloud->points[i - 4].x + _laserCloud->points[i - 3].x - 4 * _laserCloud->points[i - 2].x + _laserCloud->points[i - 1].x + _laserCloud->points[i].x;

    float ldiffY =
        _laserCloud->points[i - 4].y + _laserCloud->points[i - 3].y - 4 * _laserCloud->points[i - 2].y + _laserCloud->points[i - 1].y + _laserCloud->points[i].y;

    float ldiffZ =
        _laserCloud->points[i - 4].z + _laserCloud->points[i - 3].z - 4 * _laserCloud->points[i - 2].z + _laserCloud->points[i - 1].z + _laserCloud->points[i].z;
  ...
  for (int i = 5; i < cloudSize - 5; i++)
  {
    ...
    float dis = _laserCloud->points[i].x * _laserCloud->points[i].x + _laserCloud->points[i].y * _laserCloud->points[i].y + _laserCloud->points[i].z * _laserCloud->points[i].z;

    if (dis < thLidarNearestDis * thLidarNearestDis)
      continue;
...
/**
 * @brief 提取具有分割功能的激光点云特征，包括角点、平面点和非特征点。
 *
 * @param msg 输入的Livox点云消息，包含点的坐标、强度等信息。
 * @param laserCloud 输出的点云，包含所有点。
 * @param laserConerFeature 输出的角点特征点云。
 * @param laserSurfFeature 输出的平面特征点云。
 * @param laserNonFeature 输出的非特征点云。
 * @param msg_seg 分割结果的点云消息。
                                                        ...
                                                        const int Used_Line)
{
  ...
  // 遍历点云，将点的属性存储到 `laserCloud` 和 `data` 数组中
  for (const auto &p : msg->points)
  {
    int line_num = (int)p.line;
    if (line_num > Used_Line - 1 || p.x < 0.01 || !pcl_isfinite(p.x) || !pcl_isfinite(p.y) || !pcl_isfinite(p.z))
    {
      continue; // 跳过无效点或超过使用线束数量的点
    }
...
/**
 * @brief 提取中间帧中的激光点云特征，包括角点和平面点。
 *
 * @param msg 输入的点云消息，包含点的坐标、强度以及其他属性。
 * @param laserConerFeature 输出的角点特征点云。
 * @param laserSurfFeature 输出的平面特征点云。
 */
                                               ...
                                               pcl::PointCloud<PointType>::Ptr &laserSurfFeature)
{
  ...
  // 遍历点云中的每个点，将其分配到对应的线束容器中
  for (int i = 0; i < cloud_num; ++i)
  {
    // 从点的 `normal_y` 属性中获取线束编号，并将点索引存储到 `normal_z`
